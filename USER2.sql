...
-- 정수부 38자리, 유효숫자(소수점이하) 22자리까지 저장하는 상태
-- 특별히 값을 지정하지 않으면 가변형으로 저장되어 실제 정장되는 값만큼만
-- 공간을 차지 한다.
