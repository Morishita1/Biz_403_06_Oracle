...
SELECT sc_num, SUM(sc_score) -- 2. 그룹으로 묶인 레코드들만의 점수 합계?
FROM tbl_score
...
-- 과목(sc_subject)칼럼을 기준으로 오름차순 정령하라
