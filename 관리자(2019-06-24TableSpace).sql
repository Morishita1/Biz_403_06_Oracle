...
-- auser3TS라는 이름으로 TABLESPACE(물리적 저장공간)을 만들어라
...
-- 새로운사용자를 등록하면서 위에서 생성한 TABLESPACE에 연결하기
-- 사용자에게 TABLESPACE를 연결해 줌으로서
-- SYS 관리자의 물리적 저장곤간과 분리하여 데이터를 저장할수 있게 한다.
CREATE USER user3
IDENTIFIED BY 1234
...
-- 기존에 생성한 user2를 sys 공간에서 user3TA공간으로 변경시킨다.
-- 가급적 데이터를 추가 하지 않은상태에서 수행하는 것이 좋다.
